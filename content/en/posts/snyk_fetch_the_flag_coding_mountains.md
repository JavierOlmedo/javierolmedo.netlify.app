...
**Scripting challenges** are my favorite in CTFs, and this one has undoubtedly been my favorite from Snyk Fetch The Flag 2025. The challenge consists of **answering 50 quick questions about the most famous mountains on Earth**. We need to answer with their height (in feet) and the year they were first climbed using the following format: `height,year`.

A sample of the game start:
...
To solve this, we start with a file called `mountains.json`, which contains the answers to the questions. The goal is straightforward: automate the process to quickly answer all **50 questions**.

📄 Fragment of `mountains.json` file
...
💣 By using `coding_mountains_exploit.py`, we can efficiently solve the challenge.
